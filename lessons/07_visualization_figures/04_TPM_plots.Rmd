...
# load TPM RESULTS: 
load("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/laca6459/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/works/TPM_results/TPM_results.Rdata")

# laod DESEQ2 results
load("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/laca6459/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/works/counts_results_DESEQ/DESEQ_results.rdata")
...
avg_cols <- grep("_avg", names(Gm28438_tpm), value = TRUE)

sd_cols <- grep("_sd", names(Gm28438_tpm), value = TRUE)

# This is another way to "melt" or pivot longer
plot_df <- data.frame(
  time = as.numeric(gsub("_avg", "", avg_cols)),
  avg = unlist(Gm28438_tpm[avg_cols]),
  sd = unlist(Gm28438_tpm[sd_cols])
)
