...
# 1) DESeq2 on Long Mouse Dox Time Course DDS
```{r run DESEQ2}
# Create DDS
...
# 2) Extracts Results from DDS -{0 v 12 hrs}

results( extract the results of
  <name of dds to analyze>,
  contrast = c("time_point", "12", "0") specifies that you want to compare the "12" level of the "time_point" factor against the "0" level.
...
# 3) Turn res_0 into a Data Frame
res_0 is a S4 object and abit confusing let's turn into a data frame:
...
sum(is.na( //counting NAs
  <DataFrame>$<ColumnName>))
  
We can notice that there are NAs in the padj column let's see how many with is.na() function

```{r checking NAs}
sum(is.na(res_0_df$padj)) # counting NAs

nrow(res_0_df) # How many genes in DF

sum(is.na(res_0_df$padj)) / nrow(res_0_df) # what percentage
...
# 5) Merge in the Gene Names (g2s)

merge(<dds results>,
  <gene names>,
  by = c("merege based on" = "))
...
# 6) Filter Data Based on Significance
...
This is a good opportunity to learn some tidyverse with the pipe %>% function
...
filtered_res_0_df_2 <- res_0_df %>%
  filter(padj < 0.05, abs(log2FoldChange) > 1) #Filter genes that have a significant change and change by a 2 fold
...
#Concatinate, paste, df$gene_symbol, put a new line after each one,
