...
seqnames(gencode_gr) %>% 
  head() # View the "levels" = the chromosome names and are a meta-level
...
table(seqnames(gencode_gr)) %>%
  head() # This is telling us how many features are associated with each chromosome.

table(seqnames(gencode_gr)) %>% 
  summary() #Tells how many features are in the genome

# Let's find out how many features the mitochondrial chr has (22th level - mouse (20) has less chromosomes than humans (23)
table(seqnames(gencode_gr))[22] #Look at the table, based on the sequence, chromosome = 22 (22th layer = mitochondrial)
