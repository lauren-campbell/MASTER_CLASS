...
# Let's create a GRange using IRange logic. What we need is: Chromosome, Strand, Ranges

gr <- GRanges(seqnames = c("chrX"), #Chromosome
              strand = c("+", "-", "*"), #Strand
              ranges = IRanges(start = c(1,200, 1000), end = c(30, 299, 1200))) #Ranges
...
seqinfo(gr) #Shows Information of Genomic Range

# we can see some additional place holders we have not changed such as the 
# Genome version or if the chromosome is circular. 
# Seqlenght = the length of chromosome. 

#Adding Information!
seqlengths(gr) <- c("chrX" = 100000) #Adding Length of chromosome
seqinfo(gr)

seqlengths(gr) #Accessing seq lengths

# we see that we only have one 'level' here for ChrX
seqlevels(gr) <- c("chrX", "chrY") #Add another chromosome
seqlevels(gr) #Accessing levels (shows X & Y)

# Adding a Genome:
genome(gr) <- c("mm10")
gr # now we have a genome, chromosome and strand associated with each feature!
...
peaks_1_tsv <- read_tsv(file.path(basepath,path, "POLR2A_R1_peaks.broadPeak"), colnames = F) #Basepath,path, can be used to consolidate the file path **CON:: hard to debug if something went wrong

?findOverlaps #See what overlaps
?import

ovf <- findOverlaps(peaks1, peaks2)

length(peaks1) # 82,184 peaks
length(peaks2) # 67,549 peaks
length(ovf) # 54,170 overlap between replicate peak 1 and 2
...
*** TO PREVENT INCORRECT INTERPRETATION, SWITCH THE TO AND FROM AND SELECT THE SMALLEST OVERLAP, ***
  IF peak1 wider than peak2 --> multiple overalps will occur
